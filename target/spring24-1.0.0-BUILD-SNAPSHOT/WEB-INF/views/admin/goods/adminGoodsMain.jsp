...
function search_goods_list(fixeSearchPeriod){
	...
	i_fixedSearch_period.value=searchPeriod; //data is here
    formObj.appendChild(i_fixedSearch_period);
    document.body.appendChild(formObj); // Body --> formObj --> i_fixedSearch_period --> value --> searchPeriod
    formObj.method="get"; // get 방식으로 Request
    formObj.action="${contextPath}/admin/goods/adminGoodsMain.do"; // 동작 --> adminGoodsMain() 실행.
    formObj.submit();
}


function  calcPeriod(search_period){
	var dt = new Date(); // 생성 순간의 날짜와 시각
				...
				<c:otherwise>
					<c:forEach var="item" items="${newGoodsList }">
						<TR>
							...
							<!-- 상품이름을 클릭하면 상품디테일을 보여주는&수정할 수 있는 화면(modifyGoodsForm.do)으로 이동  -->
							<TD><a
